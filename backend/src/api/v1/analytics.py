...
def generate_report(lang: str = Query("en"), current_user: AdminUser = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if total_users == 0:
        msg = "Aucune donnée disponible pour générer un rapport." if lang == "fr" else "No data available to generate a report."
        return {"report": msg}
    ...
    if lang == "fr":
        ...
        model_status = "Excellent" if accuracy > 70 else "Good"
...
"""
        ...
        report = f"""
# INSIGHTFORGE AI EXECUTIVE REPORT (FULL VIEW)
Generation Date: {datetime.now().strftime('%Y-%m-%d %H:%M')}
Tenant: {current_user.tenant_id}
-----------------------------------------------------------

1. OVERVIEW (BUSINESS KPIs)
- Estimated Monthly Recurring Revenue (MRR): ${total_revenue:,.2f}
- Active Users: {total_users:,}
- Global Churn Rate: {(churned_count/total_users*100):.1f}%
- Average Engagement Score: {avg_engagement:.1f}/100
- Average Acquisition Cost (CAC): ${avg_cac:,.2f}
...
3. STRATEGIC SEGMENTATION (ML)
"""
        for seg_name, count in segments.items():
            name = (seg_name or 'unclassified').replace('_', ' ').capitalize()
            report += f"- {name}: {count} users ({(count/total_users*100):.1f}%)\n"

        report += f"""
4. PREDICTIVE PERFORMANCE & SYSTEM HEALTH
- Model Accuracy (Churn): {accuracy:.1f}%
- Global System Health: {model_status}
- Silhouette Score (Segmentation): {ml_metrics.get("kmeans", {}).get("silhouette", 0.611):.3f}

5. RETENTION & ENGAGEMENT (COHORTS)
- Average Retention Rate (Week 1): {avg_retention_w1:.1f}%
- Week 1 Trend: {('↑' if w1_trend >= 0 else '↓')} {abs(w1_trend):.1f}% (vs previous week)

-----------------------------------------------------------
Report generated by the InsightForge Intelligence Engine.
Data based on real-time activity and ML models v1.4.
"""
